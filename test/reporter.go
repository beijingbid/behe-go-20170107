...
// 获取媒体购买类型信息
// redis中取到的信息，用 | 切分，0位是购买类型，1位是购买价格
...
// 初始化 task_ch ，启动32个队列，每个队列最大300000条记录
func initTaskCh() {
	count = 32
	for i := 0; i < count; i++ {
...
// 到达的日志，扔到 arrive_log_cache 的队列里
// 2毫秒处理一次，如果队列中项目大于500 ，执行 arrive_process
...
func getad_process(strarr *[]string) {
	...
	if realcost > 0 && realcost < 1000000000 {
		...
		if adtype >=2 {
			//移动APP报表-------------------
			// debug 1 "APP^2017-01-04^广告主id^活动id^订单id^appid^广告位id"
			// getad 日志
...
// 拆解完的日志，进行存储
// key的格式 ： "ADRM^" + str_today + "^" + advid + "^" + str_hour + "^" + str_minute 
// key的格式 :  报表名称+日期+广告主id+其他业务上需要的key
...
// 拆解 曝光日志，并且调用 viewSetRecord 方法存储数据到map
// key的格式 ： "ADRM^" + str_today + "^" + advid + "^" + str_hour + "^" + str_minute 
// key的格式 :  报表名称+日期+广告主id+其他业务上需要的key
func view_process(strarr *[]string) {
    ...
    if adtype >=2 {
			//移动APP报表-------------------			
			// debug 2 "APP^2017-01-04^广告主id^活动id^订单id^appid^广告位id"
			// 曝光监测日志
			mb_app_key := "APP^" + str_today + "^" + advid + "^" + cid + "^" + oid + "^" + mb_appid + "^" + slotid
...
func winnotic_process(strarr *[]string) {
	...
	if realcost > 0 && realcost < 1000000000 {
		...
		if adtype >=2 {
			//移动APP报表-------------------
			// debug 3 "APP^2017-01-04^广告主id^活动id^订单id^appid^广告位id"
			// winnotice 日志
			mb_app_key := "APP^" + str_today + "^" + advid + "^" + cid + "^" + oid + "^" + mb_appid + "^" + slotid
...
// umod =1 线下购买 false
// 点击事件 > 竞价时间 2天 true
// u_+用户id 当key redis中，增量+1
// 大于3次点击的 都是 true
// 第一次点击，设定有效期2天
// debug 这里的判定是以用户id为key，没有绑定订单 广告的信息，那么直接这么判断是否过于武断
// 
// 
...
// 点击信息 用bidid做key ，序列化 存入redis 有效时间 432000 5天
...
func click_process(strarr *[]string) {
    ...
    if adtype >=2 {
			//移动APP报表-------------------
			// debug 4 "APP^2017-01-04^广告主id^活动id^订单id^appid^广告位id"
			// 点击日志
			mb_app_key := "APP^" + str_today + "^" + advid + "^" + cid + "^" + oid + "^" + mb_appid + "^" + slotid
...
// 在redis中存储用户最后存在的时间
// key : "alive_"+bhuid, (oid + "_" + sessionid) 有效期2小时
...
func arrive_process(strarr *[]string) {
    ...
    if adtype >=2 {
			//移动APP报表-------------------
			// debug 5 "APP^2017-01-04^广告主id^活动id^订单id^appid^广告位id"
			// 对应  日志中的    工作表1  
			mb_app_key := "APP^" + str_today + "^" + advid + "^" + cid + "^" + oid + "^" + mb_appid + "^" + slotid
...
// 更新记录
// 30秒处理一次  把map里的数据写入到redis里，并且更新标记
func updateRecord() {
...
// 多线程处理
	...
	initTaskCh()
	initDomainCateMap()
	// 根据传入参数的类型，调用syslog或者kafka方法，生成队列
...
/*
从文件逐行读取，并且去除  \r \n \t (只去除一次)
把获取到的值更新到  blacklistip_map  的map 里
更新之前先把 blacklistip_map 锁定。
*/
func loadfile() {
